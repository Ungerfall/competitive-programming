...
""";
...
#endif
			string one = Scanner.String();
			string another = Scanner.String();
			int[] counterOne = new int[26];
			int[] counterAnother = new int[26];
			...
			{
				int code = another[i] - 'a';
				counterAnother[code]++;
			}
			
			bool valid = true;
			for (int i = 0; i < counterOne.Length; i++)
			{
				if (counterOne[i] != counterAnother[i])
				{
					valid = false;
					break;
				}

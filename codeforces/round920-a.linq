...
""";
			...
			{
				var (xStart, yStart) = Scanner.IntInt();
				double minDist = double.MaxValue;
				foreach (var __ in Enumerable.Range(0, 3))
				{
					var (xEnd, yEnd) = Scanner.IntInt();
					double dist = Math.Sqrt(Math.Pow(xStart - xEnd, 2d) + Math.Pow(yStart - yEnd, 2d));
					minDist = Math.Min(minDist, dist);
				}
				
				Console.WriteLine((int)Math.Pow(minDist, 2d));
			}
		}
	}

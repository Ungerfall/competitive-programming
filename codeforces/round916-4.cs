...
""";
			...
			{
				int days = Scanner.Int();
				int[] skiing = Scanner.Array<int>();
				int[] movie = Scanner.Array<int>();
				int[] boardGames = Scanner.Array<int>();
				long max = 0L;
				for (int i = 0; i < days; i++)
				{
					for (int j = 0; j < days; j++)
					{
						for (int k = 0; k < days; k++)
						{
							...
							max = Math.Max(checked((long)skiing[i] + (long)movie[j] + (long)boardGames[k]), max);
						}
					}
				}

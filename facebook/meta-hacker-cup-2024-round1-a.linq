...
""";
			...
			using var stringReader = new StringReader(File.ReadAllText(@"C:\development\competitive-programming\facebook\subsonic_subway_input.txt"));
			...
			foreach (var caseNumber in Enumerable.Range(0, t))
			{
				...
				double speedMin = (double)1 / b;
				double speedMax = (double)1 / a;
				bool can = true;
				for (int i = 1; i < n; i++)
				{
					(a, b) = Scanner.IntInt();
					int s = i + 1;
					double time = s / speedMin;
					if (time < a)
					{
						can = false;
					}
					
					speedMin = Math.Max(speedMin, (double)s / b);
				...
				Console.WriteLine($"Case #{caseNumber + 1}: {(can ? speedMin : -1)}");
			}
		}
	}

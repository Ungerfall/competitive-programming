...
""";
			...
			using var stringReader = new StringReader(File.ReadAllText(@"C:\development\competitive-programming\facebook\line_by_line_input.txt"));
			Console.SetIn(stringReader);
#endif
			int t = Scanner.Int();
			foreach (var caseNumber in Enumerable.Range(0, t))
			{
				var (n, p) = Scanner.IntInt();

				double pPercent = p / 100.0;
				double goal = Math.Pow(pPercent, n - 1);
				if (goal < 1e-6)
				{
					Console.WriteLine($"Case #{caseNumber + 1}: {0.0}");
				}
				else
				{
					double PP = Math.Pow(goal, 1.0 / (double)n);
					double change = (PP - pPercent) * 100.0;
					//(new { n, p, goal, PP, change }).Dump();
					Console.WriteLine($"Case #{caseNumber + 1}: {change}");
				}
			}
		}
	}
